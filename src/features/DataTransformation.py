...
class PrincipalComponentAnalysis:
    ...
    # Get the top contributing features for the selected number of principal components.
    def get_top_contributing_features(self, data_table, cols, number_comp, top_n=4):
        # Normalize the data first.
        dt_norm = self.normalize_dataset(data_table, cols)
        ...
        # Get the loading matrix (features' contributions to the principal components).
        loading_matrix = self.pca.components_

        # Create a DataFrame to hold the feature contributions for each principal component.
        feature_contributions = pd.DataFrame(
            loading_matrix.T,
            index=cols,
            columns=[f"PC{i+1}" for i in range(number_comp)]
        )

        # Calculate the absolute contributions for each feature across all components.
        total_contributions = feature_contributions.abs().sum(axis=1)

        # Sort features by total contribution and return the top_n features.
        top_features = total_contributions.sort_values(ascending=False).head(top_n)
